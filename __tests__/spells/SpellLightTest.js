...
describe('Spell light', function () {
    describe('when in dark room', function () {
        it('should not be able to see other creatures', function () {
            const { manager: m, creatures: { c1, c2 }} = getNewManager('c-deva');
            c1.dice.cheat(0.1);
            c2.dice.cheat(0.1);
            ...
            expect(c1.getters.getEnvironment[m.CONSTS.ENVIRONMENT_DARKNESS]).toBeTruthy();
            expect(c2.getters.getEnvironment[m.CONSTS.ENVIRONMENT_DARKNESS]).toBeTruthy();
        ...
        it('should be disadvantaged on attack', function () {
            const { manager: m, creatures: { c1, c2 }} = getNewManager('c-deva', 'c-orc');
            c1.dice.cheat(0.1);
            c2.dice.cheat(0.1);
            c1.mutations.setEnvironment({ environment: m.CONSTS.ENVIRONMENT_DARKNESS, value: true });
            c2.mutations.setEnvironment({ environment: m.CONSTS.ENVIRONMENT_DARKNESS, value: true });
